...
export interface Question<TCtx extends Context = Context>
  ...
  returnable?: boolean;
  timeout?: number;
  validate?: (input: any, answers: Answer, context: TCtx) => Promise<boolean | string> | boolean | string;
  ...
  private promptWithTimeout(
    ...
    const timeout = new Promise<Answer>((resolve) => {
      setTimeout(() => {
        ...
        resolve({ [question.name]: resolve(question.default, answers, this.context) });
      }, timeoutSec * 1000);
    });
