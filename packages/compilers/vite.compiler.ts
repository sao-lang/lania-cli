...
export class ViteCompiler extends Compiler<InlineConfig, ViteDevServer, PartialVite> {
    ...
    private async mergeWithPluginOptions(config: InlineConfig, isBuildMode = false) {
        const configHook = (currentConfig: InlineConfig) => {
            return this.base.mergeConfig(currentConfig, {
        ...
        const baseConfig = this.mergeBaseConfig(
            {},
            {
                command: isBuildMode ? 'build' : 'serve',
                mode: isBuildMode ? 'production' : 'development',
            },
        );
        return await mergeConfig(
            this.base.mergeConfig({ logLevel: 'silent', plugins }, config),
            baseConfig,
        );
    }
